    ...
    function followMutate() {
        if (client) {
            client.mutate({
                ...
                variables: { followed_type: 'users', followed_id: LiveStore?.streamer?.id }
            }).then((rs: any) => {
                console.log("关注成功", rs);
    ...
    return (
        <View style={{
            ...
            <TouchableOpacity
                ...
                <Avatar uri={LiveStore?.streamer?.avatar ?? ''} size={AvatarSize} borderWidth={1} borderColor={'white'} frameStyle={{ marginTop: -25, marginBottom: 10, backgroundColor: 'orange' }} />
            </TouchableOpacity>

            <View style={{ flexDirection: 'row', justifyContent: 'center', alignItems: "center" }}>
                <Text style={{ color: "#222", marginEnd: 12, fontSize: 16 }}>{LiveStore?.streamer?.name ?? ''}</Text>
                <Image source={require('./res/level_badge.png')} resizeMode={'contain'} style={{ height: 20, width: 20, marginEnd: 6 }} />
                {LiveStore?.streamer?.level && <Text style={{ fontSize: 16, color: '#888' }}>{LiveStore?.streamer?.level?.level ?? 1}级</Text>}
            </View>
