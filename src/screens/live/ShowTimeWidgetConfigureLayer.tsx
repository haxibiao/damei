...
const ShowTimeWidgetConfigureLayer = React.memo((props:{
    navigation:any,
    startCallback:() => void
}) => {
    ...
    const StartLiveHandler = () => {
        ...
        if(client){
            console.log("newclient存在，即将调用接口");
            client.mutate({
                mutation: GQL.GetLivePushURL,
                variables: {title: "标题"}
            }).then((result:any) => {
                console.log("直播推流接口返回数据:  ",result);
            }).catch((err:any) => {
                //TODO: error 
                console.log("直播推流接口错误: ",err)
            });
        }
        //props.startCallback();
    }
...
export default ShowTimeWidgetConfigureLayer;
