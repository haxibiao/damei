...
import { HxfButton, RewardTipsOverlay, TouchFeedback } from 'components';
...
const adapterGraphData = {
    ...
    bonusExpirationTime:
        (Number(new Date(new Date().toLocaleDateString() + ' 23:59:59').getTime()) - new Date().getTime()) / 1000, // 收益重置时间
    ...
    return (
        <ScrollView style={styles.container}>
            ...
            <View style={styles.graphContainer}>
                ...
                <View style={styles.subTitle}>
                    <Image style={styles.subTitleImage} source={require('@src/assets/images/graph_red.png')} />
                    <Text style={styles.subTitleText}>邀请收益</Text>
                </View>
                <TouchFeedback
                    onPress={() => {
                        navigation.navigate('Share', { user: statistics });
                    }}>
                    <ImageBackground
                        source={require('@src/assets/images/profit_red.png')}
                        style={styles.shareYieldImage}>
                        <View style={styles.shareProfitList}>
                            <View style={styles.listItem}>
                                <Text style={styles.shareItemCount}>
                                    {Tools.NumberFormat(syncGetter('invitation.reward_total_amount', statistics) || 0)}
                                    <Text style={styles.shareItemUnit}>{` RMB`}</Text>
                                </Text>
                                <Text style={styles.shareItemName}>邀请总收益</Text>
                            </View>
                            <View style={styles.border} />
                            <View style={styles.listItem}>
                                <Text style={styles.shareItemCount}>
                                    {Tools.NumberFormat(
                                        syncGetter('invitation.reward_total_share_bonus', statistics) || 0,
                                    )}
                                    <Text style={styles.shareItemUnit}>{` RMB`}</Text>
                                </Text>
                                <Text style={styles.shareItemName}>徒弟提现分红</Text>
                            </View>
                            <View style={styles.border} />
                            <View style={styles.listItem}>
                                <Text style={styles.shareItemCount}>
                                    {`${statistics.invitations_success_count || 0}`}
                                    <Text style={styles.shareItemUnit}>{` 人`}</Text>
                                </Text>
                                <Text style={styles.shareItemName}>成功邀请数</Text>
                            </View>
                        </View>
                    </ImageBackground>
                </TouchFeedback>
                ...
                <View style={styles.yieldItem}>
                    ...
                    <ImageBackground source={require('@src/assets/images/profit_violet.png')} style={styles.yieldImage}>
            ...
            <View style={styles.ruleContainer}>
                <View style={[styles.subTitle, { marginBottom: PxFit(10) }]}>
                    <Image style={styles.remindIcon} source={require('@src/assets/images/participation_remind.png')} />
                ...
                <Text style={styles.ruleText}>
                    {`    1、分红股与您在平台的个人行为和每日活跃度、贡献值、邀请好友数几大维度动态有关，邀请数与贡献值越高的用户将有机会获得更多分红股。其中个人行为包括但不限于：邀请好友数、登录天数、签到天数、在线时长、答题数、出题数、任务完成情况等因素。`}
                </Text>
                <View style={styles.ruleItem}>
                    <View style={styles.ruleItemCircle} />
...
const shareYieldImageWidth = SCREEN_WIDTH - PxFit(Theme.itemSpace) * 2;
...
const styles = StyleSheet.create({
    ...
    shareYieldImage: {
        ...
        height: (shareYieldImageWidth * 270) / 774,
        justifyContent: 'center',
        marginBottom: PxFit(30),
        marginTop: PxFit(10),
        overflow: 'hidden',
        width: shareYieldImageWidth,
    },
    subTitle: {
        alignItems: 'center',
        flexDirection: 'row',
