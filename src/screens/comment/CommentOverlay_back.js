...
class CommentOverlay extends Component {
    ...
    // _renderAskQuestionComment = comments => {
    //     if (comments && comments.length === 0) return null;
    //     const { question } = this.props;
    //     console.log('question', question);
    //     // TODO： 需要一个查询回答列表的query   comment所需字段  已被采纳   askQuestion 增加字段  已完成回答采纳
    //     return (
    //         <View>
    ...
    render() {
        const { onHide, question, hideComment } = this.props;
        const {
            visible,
            offset,
            modalVisible,
        ...
        console.log('show overlay', question, visible);
        if (!visible || !question) {
            return <View />;
        }
        return (
