...
// Get Specify the date
export function GetSpecifyDate(dayCount) {
    const current = new Date();
    current.setDate(current.getDate() + dayCount);
    return current;
}
...
export function regular(account) {
    ...
    const value = phoneReg.test(account) || mailReg.test(account);
    return value;
}
